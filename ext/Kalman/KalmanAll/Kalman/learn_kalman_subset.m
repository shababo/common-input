function [A, C, Q, R, initx, initV, LL] = ...
    learn_kalman_subset(data, A, C, Q, R, initx, initV, max_iter, diagQ, diagR, ARmode, model, constr_fun, varargin)
% LEARN_KALMAN Find the ML parameters of a stochastic Linear Dynamical System using EM.
%
% [A, C, Q, R, INITX, INITV, LL] = LEARN_KALMAN_SUBSET(DATA, A0, C0, Q0, R0, INITX0, INITV0) fits
...
while ~converged && (num_iter <= max_iter) 
  ...
  for ex = 1:N
	...
	Estep(y, A, C, Q, R, initx, initV, ARmode, model);
    beta = beta + beta_t;
    gamma = gamma + gamma_t;
    delta = delta + delta_t;
